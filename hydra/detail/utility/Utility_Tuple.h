...
namespace hydra {
	...
	struct tuple_type {
		...
		typedef  decltype(hydra::thrust::tuple_cat(hydra::thrust::tuple<T>(),
				typename tuple_type<N-1, T>::type())) type;
	};

	template <typename T>
	struct tuple_type<0, T> {
		typedef decltype(hydra::thrust::tuple<>()) type;
	};
    */
	...
	{
		typedef hydra::thrust::tuple<T1..., T2...> type;
	};

	template<typename ...T1,typename ...T2>
	struct tuple_cat_type<hydra::thrust::detail::tuple_of_iterator_references<T1...>,
	hydra::thrust::detail::tuple_of_iterator_references<T2...> >
	{
		typedef hydra::thrust::detail::tuple_of_iterator_references<T1..., T2...> type;
	};
	...
	struct CompareTuples{
		...
		{
			return COM< typename hydra::thrust::tuple_element<N, Tuple1>::type>()(hydra::thrust::get<N>(t1) , hydra::thrust::get<N>(t2) );
		}
	...
	template <size_t N, typename T>
	struct tuple_type {
		typedef typename repeat<T, N, hydra::thrust::tuple>::type type;
	};

	template <size_t N, typename T>
	struct references_tuple_type {
		typedef typename repeat<T, N, hydra::thrust::detail::tuple_of_iterator_references>::type type;
	};
	...
	max_helper( T const& tuple, C& max_value, size_t& max_index){
	 ...
	 max_value = max_value > hydra::thrust::get<I>(tuple) ? max_value : hydra::thrust::get<I>(tuple);
	...
	size_t max(hydra::thrust::tuple<T...> const& tuple){

	typedef typename std::common_type<T...>::type C;

	 size_t i=0;
	 C max_value = hydra::thrust::get<0>(tuple);

	 max_helper<hydra::thrust::tuple<T...>,C,sizeof...(T)>(tuple, max_value, i);
	...
	{
		return hydra::thrust::make_zip_iterator( hydra::thrust::make_tuple( array_of_iterators[Index]...) );
	}
	...
	{
		return hydra::thrust::make_zip_iterator(hydra::thrust::make_tuple(head , array_of_iterators[Index]...));
	}
	...
	auto make_rtuple_helper(hydra::thrust::tuple<T...>& t , index_sequence<I...>)
	-> hydra::thrust::tuple<T&...>
	{ return hydra::thrust::tie(hydra::thrust::get<I>(t)...) ;}

	template<typename ...T>
	auto  make_rtuple( hydra::thrust::tuple<T...>& t )
	-> hydra::thrust::tuple<T&...>
	{
	...
	_get_element(const size_t index, T& t, R*& ptr )
	{
	    ...
	    index==I ? ptr=&hydra::thrust::get<I>(t):0;
	...
	__hydra_host__  __hydra_device__ inline
	R& get_element(const size_t index, hydra::thrust::tuple<T...>& t)
	{
	    R* ptr;
	...
	__hydra_host__  __hydra_device__ inline
	void split_tuple_helper(hydra::thrust::tuple<T1...> &t1, hydra::thrust::tuple<T2...> &t2,
			hydra::thrust::tuple<T1..., T2...> const& t , index_sequence<I1...>, index_sequence<I2...>)
	{
		t1 = hydra::thrust::tie( hydra::thrust::get<I1>(t)... );
		t2 = hydra::thrust::tie( hydra::thrust::get<I2+ sizeof...(T1)>(t)... );

	}

	template< typename ...T1,  typename ...T2>
	__hydra_host__  __hydra_device__ inline
	void split_tuple(hydra::thrust::tuple<T1...> &t1, hydra::thrust::tuple<T2...> &t2,
	...
	__hydra_host__  __hydra_device__ inline
	auto split_tuple_helper(hydra::thrust::tuple<T...> &t, index_sequence<I1...>, index_sequence<I2...>)
	-> decltype( hydra::thrust::make_pair(hydra::thrust::tie( hydra::thrust::get<I1>(t)... ), hydra::thrust::tie( hydra::thrust::get<I2+ + sizeof...(I1)>(t)... ) ) )
	{
		auto t1 = hydra::thrust::tie( hydra::thrust::get<I1>(t)... );
		auto t2 = hydra::thrust::tie( hydra::thrust::get<I2+ sizeof...(I1)>(t)... );

		return hydra::thrust::make_pair(t1, t2);
	}

	template< size_t N, typename ...T>
	__hydra_host__  __hydra_device__ inline
	auto split_tuple(hydra::thrust::tuple<T...>& t)
	...
	template<typename ...T, size_t... I1, size_t... I2 >
	__hydra_host__  __hydra_device__ inline
	auto split_tuple_helper(hydra::thrust::tuple<T...>  const& t, index_sequence<I1...>, index_sequence<I2...>)
	-> decltype( hydra::thrust::make_pair(hydra::thrust::tie( hydra::thrust::get<I1>(t)... ), hydra::thrust::tie( hydra::thrust::get<I2+ + sizeof...(I1)>(t)... ) ) )
	{
		auto t1 = hydra::thrust::tie( hydra::thrust::get<I1>(t)... );
		auto t2 = hydra::thrust::tie( hydra::thrust::get<I2+ sizeof...(I1)>(t)... );
	...
	template< size_t N, typename ...T>
	__hydra_host__  __hydra_device__ inline
	auto split_tuple(hydra::thrust::tuple<T...> const& t)
	-> decltype( split_tuple_helper( t, make_index_sequence<N>{}, make_index_sequence<sizeof...(T)-N>{} ) )
	{
		return split_tuple_helper( t, make_index_sequence<N>{}, make_index_sequence<sizeof...(T)-N>{} );
	}
	...
	__hydra_host__ __hydra_device__
	auto dropFirstHelper(hydra::thrust::tuple<Head, Tail...> const& t  , index_sequence<Is...> )
	-> hydra::thrust::tuple<Tail...>
	{
		return hydra::thrust::tie( hydra::thrust::get<Is+1>(t)... );
	}

	template< typename Head,  typename ...Tail>
	__hydra_host__ __hydra_device__
	auto dropFirst( hydra::thrust::tuple<Head, Tail...> const& t)
	...
	auto changeFirstHelper(T& new_first, hydra::thrust::tuple<Head, Tail...>  const& t  , index_sequence<Is...> )
	-> hydra::thrust::tuple<T,Tail...>
	{
		return hydra::thrust::make_tuple(new_first, hydra::thrust::get<Is+1>(t)... );
	}

	template<typename T, typename Head,  typename ...Tail>
	auto changeFirst(T& new_first, hydra::thrust::tuple<Head, Tail...>  const& t)
	...
	-> decltype(hydra::thrust::make_tuple(passthrough<T,Is>(std::forward<T>(value))...))
	{
		return hydra::thrust::make_tuple(passthrough<T,Is>(std::forward<T>(value))...);
	}
	...
	{
		return hydra::thrust::make_tuple(Array[Is]...);
	}
	...
	{
		return hydra::thrust::make_tuple(Array[Indices]...);
	}
...
*/
	...
	// set a std::array with tuple values
	 ...
	 assignTupleToArray(hydra::thrust::tuple<Head, Tail...> const& Tuple, Type (&Array)[sizeof...(Tail)+1])
	 {
		 ...
		 Array[I] = hydra::thrust::get<I>(Tuple);
	 ...
	 std::conditional<hydra::thrust::detail::is_same<
	 typename hydra::thrust::detail::remove_const<
	     typename hydra::thrust::detail::remove_reference<A>::type
	 >::type,
	 typename hydra::thrust::detail::remove_const<
	   typename hydra::thrust::detail::remove_reference<
	      typename hydra::thrust::tuple_element<I-1, Tp>::type
	   >::type
	 >::type>::value, is_homogeneous_base<A, Tp, I-1>,
	 hydra::thrust::detail::false_type>::type{ };

	 template<typename A, typename Tp>
	 struct is_homogeneous_base<A, Tp, 0>: hydra::thrust::detail::true_type{ };

	 template<typename A, typename Tuple>
	 struct is_homogeneous:  is_homogeneous_base<A, Tuple, hydra::thrust::tuple_size<Tuple>::value > {};
	 ...
	 eval_tuple_element(Return_Type& r, int index, hydra::thrust::tuple<Tp...> const& t, ArgType const& arg)
	 {
		 if (index == 0)
		 {
			 r = (Return_Type) hydra::thrust::get<I>(t)(std::forward<ArgType const>(arg));
			 return;
		 }
	 ...
	 sum_tuple(Return_Type& r, hydra::thrust::tuple<Tp...>&& t, ArgType&& arg)
	 {
		 ...
		 sum_tuple<I + 1, Return_Type, ArgType, Tp...>( r , std::forward< hydra::thrust::tuple<Tp...> >(t), std::forward<ArgType>(arg) );
	 }
	 ...
	 sum_tuple2(Return_Type& r, hydra::thrust::tuple<Tp...>&& t, ArgType1&& arg1, ArgType2&& arg2)
	 {
		 ...
		 sum_tuple2<I + 1, Return_Type, ArgType1, ArgType2, Tp...>( r , std::forward< hydra::thrust::tuple<Tp...> >(t)
	 ...
	 multiply_tuple(Return_Type& r, hydra::thrust::tuple<Tp...>&& t)
	 {
		 ...
		 multiply_tuple<I + 1, Return_Type, Tp...>( r , std::forward< hydra::thrust::tuple<Tp...> >(t) );
	 }
	 ...
	 {
		 r = r*( (Return_Type) hydra::thrust::get<I>(std::forward< hydra::thrust::tuple<Tp...> >(t))(std::forward<ArgType>(arg)));
		 product_tuple<I + 1, Return_Type, ArgType, Tp...>( r , std::forward< hydra::thrust::tuple<Tp...> >(t), std::forward<ArgType>(arg) );
	 }
	 ...
	 product_tuple2(Return_Type& r, hydra::thrust::tuple<Tp...>&& t, ArgType1&& arg1, ArgType2&& arg2)
	 {
		 ...
		 product_tuple2<I + 1, Return_Type, ArgType1, ArgType2,  Tp...>( r ,
				 std::forward< hydra::thrust::tuple<Tp...> >(t), std::forward<ArgType1>(arg1), std::forward<ArgType2>(arg2)) ;
	 }
...
/*
	 ...
	 -> decltype(hydra::thrust::make_tuple(hydra::thrust::get<index>(tup)(n,x)...))
	 {
		 return hydra::thrust::make_tuple(hydra::thrust::get<index>(tup)(n,x)...);
	 }

	 template< typename Tup, typename ArgType>
	 __hydra_host__  __hydra_device__
	 inline auto invoke(size_t n, ArgType* x, Tup& tup)
	 -> decltype(invoke_helper2(n, x, tup, make_index_sequence< hydra::thrust::tuple_size<Tup>::value> { }))
	 {
...
*/
	...
	// __hydra_thrust_exec_check_disable__
	 ...
	 {
		 return hydra::thrust::make_tuple(hydra::thrust::get<index>(tup).GetNorm()*
				 hydra::thrust::get<index>(tup)(x)...);
	 }
	...
	// __hydra_thrust_exec_check_disable__
	 ...
	 {
		 return hydra::thrust::make_tuple(hydra::thrust::get<index>(std::forward<Tup>(tup)).GetNorm()
				 *hydra::thrust::get<index>(std::forward<Tup>(tup))(std::forward<ArgType>(x))...);
	 }

	// __hydra_thrust_exec_check_disable__
	 template< typename Tup, typename ArgType,
	 	 size_t N=hydra::thrust::tuple_size<typename hydra::thrust::detail::remove_reference<Tup>::type>::value>
	 __hydra_host__  __hydra_device__
	 ...
	 -> decltype(hydra::thrust::make_tuple(hydra::thrust::get<index>(std::forward<Tup>(tup))(std::forward<ArgType>(x))...))
	 {
		 return hydra::thrust::make_tuple(hydra::thrust::get<index>(std::forward<Tup>(tup))(std::forward<ArgType>(x))...);
	 }
	 ...
	 -> decltype(hydra::thrust::make_tuple(hydra::thrust::get<index>(tup)(x)...))
	 {
		 return hydra::thrust::make_tuple(hydra::thrust::get<index>(tup)(x)...);
	 }
	 ...
	 {
		 return  hydra::thrust::make_tuple(hydra::thrust::get<index>(std::forward<Tup>(tup))(std::forward<ArgType1>(x), std::forward<ArgType2>(y))...);
	 }
	 ...
	 {
		 return  hydra::thrust::make_tuple(hydra::thrust::get<index>(tup)(x,y)...);
	 }
	 ...
	 {
		 Return_Type r=TypeTraits<Return_Type>::one();
		 constexpr size_t N=hydra::thrust::tuple_size< hydra::thrust::tuple<Tp...>>::value;
	 ...
	 multiply_array_tuple_helper(GReal_t (&fCoefficients)[sizeof...(I)],Tuple const& tpl,
			 index_sequence<I...>)
	 {
		 return hydra::thrust::make_tuple(fCoefficients[I]*hydra::thrust::get<I>(tpl)... );
	 }
