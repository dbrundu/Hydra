...
public:
	...
	inline result_type operator()(T&& rule_abscissa)
	{
        ...
        auto rule5_weight            = hydra::thrust::get<0>(rule_abscissa);
        auto rule7_weight            = hydra::thrust::get<1>(rule_abscissa);
		auto four_difference_weight  = hydra::thrust::get<2>(rule_abscissa);
		int four_diff_index          = hydra::thrust::get<3>(rule_abscissa);
	...
	//	std::cout<< four_diff_index<<std::endl;
		...
		_temp[0]      = fval*hydra::thrust::get<0>(rule_abscissa);//w5;
		_temp[1]      = fval*hydra::thrust::get<1>(rule_abscissa);//w7;
	    if(four_diff_index>=0 && four_diff_index<int(N)){
	    	_temp[four_diff_index+2]  = fval*hydra::thrust::get<2>(rule_abscissa);//w_four_diff;
	    }
	    else if(four_diff_index==N){
	    	for(size_t i =0; i<N ;i++)
	    		_temp[i+2]  = fval*hydra::thrust::get<2>(rule_abscissa);
	    }
	...
	*/
	...
	get_transformed_abscissa_helper(  Abscissa const& abscissa, TransAbscissa& transformed_abscissa  ){
		...
		hydra::thrust::get<I>(transformed_abscissa)  =
				fA[I]*hydra::thrust::get<I+4>( abscissa )+ fB[I];
	...
	typename hydra::detail::tuple_type< hydra::thrust::tuple_size<Abscissa>::value-4
	, double>::type	{

		constexpr size_t _N = hydra::thrust::tuple_size<Abscissa>::value;
...
//-----------------------
	...
	get_dim_helper( T const& X, int& result,  int& found_zeros ){
		...
		found_zeros  += int(!(hydra::thrust::get<I+3>(X)>0.0 || hydra::thrust::get<I+3>(X) < 0.0));
	...
	int get_dim( T const& X){
		...
		return found==int(N-1)?result: found==int(N)? int(N):-1 ;//< int(hydra::thrust::tuple_size<T>::value) ? result : -1;
	}
...
struct ProcessGenzMalikBinaryCall:
		public hydra::thrust::binary_function< typename hydra::detail::tuple_type<N+2, GReal_t>::type ,
...
struct ProcessGenzMalikBox
{
	...
	inline void operator()(GenzMalikBox<N>& hyperbox)
	{
		...
		typedef typename hydra::thrust::iterator_system<RuleIterator>::type System;
		using hydra::thrust::transform_reduce;
